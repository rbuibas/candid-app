...
- **Post-event download:** save posts to the **camera roll** (`expo-media-library`, full access) for single posts (`PostViewerModal`) and bulk (`src/features/download/bulkDownload.ts` — sequential, abortable, re-mints expired signed URLs per page via `GET /posts/{id}`). Album-filing is **batched once** at the end of a run, not per item, so the OS "modify" consent fires once for the set, not per photo. Capture stays live-only regardless of the read grant — there is no image picker in the app. "Downloaded" state is **local-only** (persisted Zustand keyed by `post_id`), never a backend column. The retention banner is a nudge off the server-computed `retention_purge_at`; there is no purge job.
